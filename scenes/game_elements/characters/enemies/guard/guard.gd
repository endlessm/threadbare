...
@tool
class_name Guardicopia extends CharacterBody2D
...
func _ready() -> void:
	# 👉 AQUÍ EXACTAMENTE (al inicio)
	if get_parent().has_node("Guard2-RunningPath"):
		patrol_path = get_parent().get_node("Guard2-RunningPath")
