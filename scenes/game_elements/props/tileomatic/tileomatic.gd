...
@tool
extends Node
## Automatically paint water, foam, and shadows in a group of TileMapLayers
##
## It is very tedious and error-prone to paint these by hand. This tool assumes
## that that floor & elevation tiles should be surrounded by water, and that
## water should otherwise not be present. It also deals with using alternative
## water/cliff tiles as needed for bridges and the back row of elevated terrain
## extending over water. It is extremely hardcoded to our core TileSets and does
## not attempt to be general at all.
##
## @experimental This is only a prototype. You should expect to have to manually
## tweak this script and/or the scene if you want to use it.
...
# TODO: Should this be an array of pairs?
@export var bridge_shadows: TileMapLayer
## For bridges that are not at ground level, set a "height" metadata with integer value
## for the height above ground level. This allows the shadows to be offset correctly.
@export var bridges: Array[TileMapLayer]

# TODO: Support multiple layers of elevation
...
func _get_layer_offset(layer: TileMapLayer) -> Vector2i:
	...
	return Vector2i(0, 1) * (layer.get_meta(HEIGHT_KEY, default) as int)
		...
		for coords: Vector2i in b.get_used_cells():
			...
			match bridge_atlas_coords:
				Vector2i(0, 0):  # Horizontal left anchor
					# TODO: One alt is not enough. For the bottom-right corner
					# cliff tile, for example, we need 3 alts: one with no
					# collisions on the right, one with no collisions on the
					# bottom, and one with neither set of collisions, to handle
					# the cases of bridges coming from each edge. The one-tile
					# clifftop actually needs 2 ** 4 - 1 alts I think!... Could
					# do it as a bitmask: label the cardinal directions 0, 1, 2,
					# 3, then set the (1 << direction)th bit for each bridge,
					# and now use that numbered alternative.
					elevation_alt = 1
				...
				Vector2i(0, 3):  # Bottom of vertical bridge
					# (TODO: and how about pontoons in other directions?! Need
					# more water alts - or to put the physics into the bridge.)
