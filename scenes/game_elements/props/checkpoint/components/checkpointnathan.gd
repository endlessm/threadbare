...
func _on_interaction_started(player: Player, from_right: bool) -> void:
	...
	# GUARDAR LUGAR DE RETORNO
	var logic = get_tree().get_first_node_in_group("stealth_logic")
	if logic:
		logic.current_spawn_point = owner.get_path_to(spawn_point)
		logic.return_scene = get_tree().current_scene.scene_file_path

	# CAMBIAR A LA ESCENA DEL MINIJUEGO
	var minigame_scene: String = "res://scenes/quests/story_quests/nathan/1_nathan_stealth/nathan_stealth.tscn" 
	SceneSwitcher.change_to_file_with_transition(
		minigame_scene,
		"",  # no necesitas spawn point dentro del minijuego
