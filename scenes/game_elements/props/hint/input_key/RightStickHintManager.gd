...
const DEVICE_MAP := {
	...
	"playstation": "res://scenes/game_elements/props/hint/resources/stickmovements/stick_playstation.tres",
	...
	"steamdeck": "res://scenes/game_elements/props/hint/resources/stickmovements/stick_steamdeck.tres"
}

# Fallback genérico
const GENERIC_RESOURCE := "res://scenes/game_elements/props/hint/resources/stickmovements/stick_xbox.tres"
...
func get_texture_for(device: String, action_name: String) -> Texture2D:
	var res: JoypadButtonTexturesAim = _resource_for_device(device)
	if res:
	...
	var generic_res: JoypadButtonTexturesAim = (
		ResourceLoader.load(GENERIC_RESOURCE) as JoypadButtonTexturesAim
	)
	if generic_res:
