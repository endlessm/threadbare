...
func _ready():
	if not guard and get_parent() is Guard:
		guard = get_parent()

func _process(delta: float) -> void:
	if still_time_left_in_seconds > 0.0:
		still_time_left_in_seconds -= delta
		if still_time_left_in_seconds <= 0.0:
			still_time_finished.emit()

	guard.velocity = velocity(delta)
	guard.move_and_slide()

	if not _destination_reached and guard.global_position.distance_to(destination) <= guard.velocity.length() * delta:
...
func velocity(delta: float) -> Vector2:
