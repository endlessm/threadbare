...
func get_pieces_at(grid_position: Vector2i, groups: PieceGroups = null, layer: int = -1) -> Array[Piece2D]:
    _update_cells()
...
func get_piece(groups: PieceGroups = null, layer: int = -1, include_inactive: bool = false) -> Piece2D:
...
func get_pieces(groups: PieceGroups = null, layer: int = -1, include_inactive: bool = false) -> Array[Piece2D]:
...
func count_pieces(groups: PieceGroups = null, layer: int = -1, include_inactive: bool = false) -> int:
    var filtered_by_group: bool = PieceGroups.is_valid(groups)
    # We just want all pieces
    if not filtered_by_group:
        return _pieces.size()
