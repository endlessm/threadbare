...
func _take_turn(direction_2d: Vector2i) -> void:
    ...
    for attempt in move_attempts:
        if attempt in total_checked_attempts: continue
...
func _attempt_rule_on_piece(rule: SokobanRule, piece: Piece2D, move_attempts: Array[MoveAttempt]) -> void:
    if not rule.match_pattern: return
    if rule.match_pattern.size() == 0: return
...
func _apply_fragment(fragment: RuleFragment, piece: Piece2D, move_attempts: Array[MoveAttempt]) -> void:
...
func _is_fragment_matching(fragment: RuleFragment, piece: Piece2D, move_attempts: Array[MoveAttempt]) -> bool:
    if not piece.groups.has_group(fragment.group): return false

    if fragment.move_context != RuleFragment.MOVE.ANY:
        for attempt in move_attempts:
            if attempt.piece == piece:
                return fragment.is_matching_movement(attempt.new_grid_position - attempt.old_grid_position)
...
class MoveAttempt:
    ...
    func _init(i_piece: Piece2D, i_old_grid_position: Vector2i, i_new_grid_position: Vector2i) -> void:
        piece = i_piece
        old_grid_position = i_old_grid_position
        new_grid_position = i_new_grid_position

    func can_move(board: Board2D, other_attempts: Array[MoveAttempt], checked_attempts: Array[MoveAttempt]) -> bool:
