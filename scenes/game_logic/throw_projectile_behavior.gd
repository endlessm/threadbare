...
enum AddMode {
	...
	CHILD_OF_REFERENCE,
	## Add projectiles as siblings of the reference node.
	SIBLING_OF_REFERENCE,
	## Add projectiles to the current scene root.
	CHILD_OF_ROOT,
}
## The projectile scene to instantiate when throwing a projectile.
...
@export_range(10., 100., 5., "or_greater", "or_less", "suffix:m/s") var impulse: float = 30.0

## Where to add the projectiles.
@export var add_mode: AddMode

## Projectiles may be added as children or siblings of this node
## depending on [member add_mode].
## [br][br]
...
func throw() -> void:
	...
	match add_mode:
		...
		AddMode.SIBLING_OF_REFERENCE:
			reference.add_sibling(projectile)
