...
static func expose_children_property(
	...
	var children: Array = parent.find_children("*", type)
	for child: Node in children:
		var property_dict: Dictionary = get_property_dict(child, property)
		if not property_dict.is_empty():
			properties.push_back(
				{
					"name": "%s__%s" % [child.name.to_snake_case(), property],
					"type": property_dict["type"],
					"class_name": property_dict.get("class_name"),
					"hint": property_dict.get("hint"),
					"hint_string": property_dict.get("hint_string"),
					"usage": property_dict.get("usage")
				}
			)
