...
func _on_timeout() -> void:
	...
	ink_blob.ink_color_name = randi_range(0, 3)
	ink_blob.global_position = (
		ink_blob_marker.global_position + ink_blob.direction.normalized() * 20.
	)
	Engine.get_main_loop().current_scene.add_child(ink_blob)
	await animated_sprite_2d.animation_looped
...
func explode(ink_color_name: int = 0) -> void:
	var big_splash: BigSplash = BIG_SPLASH.instantiate()
	big_splash.ink_color_name = ink_color_name
	Engine.get_main_loop().current_scene.add_child(big_splash)
