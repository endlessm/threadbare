...
func _on_Area2D_body_exited(body: Node) -> void:
	if body.is_in_group("player"):
		...
		player_node = null
		print("🚶‍♂️ Jugador salió del rango")
		$ShootTimer.stop()

func _on_shoot_timer_timeout() -> void:
	print("⏱️ Timer activado")
	if not player_in_range or is_attacking or is_dead or is_dialogue_running or is_returning:
		print("Jefe ocupado, no dispara")
		$ShootTimer.start()
		return
	is_attacking = true
	var attack_choice: float = randf()
	if attack_choice < chance_dash:
		print(">> ATAQUE: Embestida")
		await dash_attack()
	elif attack_choice < chance_shotgun:
		print(">> ATAQUE: Escopeta")
		shoot_shotgun()
		await get_tree().create_timer(0.5).timeout 
	elif attack_choice < chance_circle:
		print(">> ATAQUE: Círculo")
		shoot_circle()
		await get_tree().create_timer(0.5).timeout
	else:
		print(">> ATAQUE: Simple")
...
func take_damage(amount: int) -> void:
	...
	hits_taken += 1
	print("💥 Jefe recibió ", amount, " de daño. Vida: ", current_health)
	invulnerable = true
	$InvulTimer.start()
	blink()
	if current_health <= 0:
