...
@onready var tilemap_puente: TileMapLayer = $"../TileMapLayers/Puente"
@onready var tilemap_bloqueo: TileMapLayer = $"../TileMapLayers/AguaSolida"
@onready var tilemap_agua_decorativa: TileMapLayer = $"../TileMapLayers/AguaDecorativa"
var ID_SOURCE_BLOQUEO: int = 0
var COORDS_TILE_BLOQUEO: Vector2i = Vector2i(4, 2) 
...
func shoot_triangle_attack() -> void:
	...
	get_tree().current_scene.call_deferred("add_child", special_bullet)
...
func destruir_puente() -> void:
	print("¡Destruyendo el puente y añadiendo colisión al agua!")
	var celdas_del_puente: Array = tilemap_puente.get_used_cells()
	for celda: Vector2i in celdas_del_puente:
		tilemap_puente.set_cell(celda, -1)
		var source_id: int = tilemap_agua_decorativa.get_cell_source_id(celda)
		var atlas_coords: Vector2i = tilemap_agua_decorativa.get_cell_atlas_coords(celda)
		if source_id != -1:
			tilemap_bloqueo.set_cell(
				celda,
				source_id,
				atlas_coords
			)
