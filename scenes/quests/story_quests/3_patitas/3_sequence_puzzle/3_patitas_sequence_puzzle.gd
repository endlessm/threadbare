...
# Variables para el efecto de vibración
...
# Nodos (Asegúrate de que las rutas coincidan con tu Árbol de Escenas)
@onready var linterna_gato = $OnTheGround/Player/PointLight2D2
@onready var cohete = $CoheteDespegue

# --- INTERFAZ UI ---
# Cambia estas rutas si tus nodos están en otro lugar
@onready var aviso_error : Label = $CanvasLayer/AvisoError 
@onready var panel_final : Panel = $CanvasLayer/PanelFinal
@onready var label_mensaje_final : Label = $CanvasLayer/PanelFinal/LabelMensaje

func _ready() -> void:
	# Inicializamos la interfaz como invisible
	if aviso_error: aviso_error.visible = false
	if panel_final: panel_final.visible = false
	...
	# Conexión de señales de los cristales
...
# --- SISTEMA DE AVISO DE ERROR ---
func _mostrar_aviso_orden() -> void:
	if aviso_error:
		aviso_error.text = "¡Esa piedra no reacciona... debo buscar la correcta!"
		aviso_error.visible = true
		await get_tree().create_timer(2.0).timeout
		aviso_error.visible = false

# --- SISTEMA DE FINALIZACIÓN ---
func _ejecutar_final() -> void:
	if cohete:
		cohete.visible = true
		debe_vibrar = true 
	
	# Activamos el panel de éxito
	if panel_final:
		label_mensaje_final.text = "¡Excelente! Hemos recolectado todos los cristales lunares necesarios en el orden correcto. ¡Despeguemos!"
		panel_final.visible = true
	
	# Esperamos antes de cambiar de escena
