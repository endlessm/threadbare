...
func actualizar_color():
	match hilos_totales:
		0: canvas_modulate.color = Color(0.4, 0.4, 0.5)
		1: canvas_modulate.color = Color(0.6, 0.6, 0.7)
		2: canvas_modulate.color = Color(0.8, 0.8, 0.9)
		3: 
			canvas_modulate.color = Color(1.0, 1.0, 1.0)
			# Encendemos la flecha guía cuando se tiene el poder total
			if flecha:
				flecha.visible = true
...
func _on_zona_climax_body_entered(body: Node2D) -> void:
	if body.is_in_group("player"):
		...
		if hilos_totales >= 3:
			...
			SceneSwitcher.change_to_file_with_transition("res://scenes/quests/story_quests/el_hilo_que_no_teji/2_combat/el_hilo_que_no_teji_combat.tscn")
