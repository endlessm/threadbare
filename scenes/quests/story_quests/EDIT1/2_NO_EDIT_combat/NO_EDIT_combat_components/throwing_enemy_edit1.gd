...
func shoot_projectile_at(target: Node2D) -> bool:
	...
	var bullet_count_copy : int = bullet_count
	var target_label: String = allowed_labels.pick_random()

	var separation_angle := PI / separation
	var start_angle := -separation_angle / 2
	var global_angle := projectile_marker.global_position.direction_to(target.global_position)
