...
func vec_rotate(angle, vector: Vector2) -> Vector2:
	var c := cos(-angle)
	var s := sin(-angle)
	return Vector2(vector.x * c - vector.y * s, vector.y * c + vector.x * s)

func vec_angle(vector: Vector2) -> float:
...
## for periodic throws.
func throw() -> void:
	var additive := Vector2(0, 80)
	var count := 0
	var changed := false
	var direction
	var none = Vector2(0, 1)
	var vel
	for bullet in projectile_scenes:
		var projectile := bullet.instantiate() as Node2D
		var aimX := target.global_position - global_position
		if count == 0:
		...
		if projectile is RigidBody2D:
			projectile.apply_impulse(vel * impulse)
		match add_mode:
